...
def test_gauss_method_line(test_data_line):
    """Проверка расчета методом Гаусса для сети, содержащей только линии"""
    ...
    nodes, branches = GaussMethod.run(nodes, branches, parameters)

    assert abs(nodes[1].voltage - complex(115.716, 0.264)) < 1e-3
    assert abs(nodes[2].voltage - complex(109.989, -4.333)) < 1e-3

def test_gauss_method_t2(test_data_t2):
    """Проверка расчета методом Гаусса для сети, содержащей только двухобмоточный трансформатор"""
...
def test_gauss_method_t3(test_data_t3):
    """Проверка расчета методом Гаусса для сети, содержащей только трехобмоточный трансформатор"""
    # проверка правильности расчета
